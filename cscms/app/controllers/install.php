	...
	public function save5()
	{
            ...
            if(file_exists(FCPATH.'packs/install/install.lock')){
                ...
                if(empty($web_name)||empty($web_url)||empty($web_path)||empty($admin_name)||empty($admin_pass)||empty($admin_code)) msg_url('<font color=red>请把数据填写完整！</font>','javascript:history.back();');
                    ...
                    //修改配置文件
	                $config=read_file("./cscms/lib/Cs_Config.php");
	                ...
	                if(!write_file('./cscms/lib/Cs_Config.php', $config)){
                            msg_url('<font color=red>文件./cscms/lib/Cs_Config.php，没有写入权限！</font>','javascript:history.back();');
                            exit();
					}
                    //写入管理员
		            $this->load->model('CsdjDB');
            ...
            if(file_exists(FCPATH.'packs/install/install.lock')){
                ...
                if(!$lnk) {
				   ...
				   if(mysql_select_db($dbname)){
                        ...
                        //修改数据库配置文件
	                    $config=read_file("./cscms/lib/Cs_DB.php");
	                    ...
	                    if(!write_file('./cscms/lib/Cs_DB.php', $config)) exit('5');
