...
class Gbook extends Cscms_Controller {
	...
	public function hf()
	{
			...
			if($id==0) admin_msg(L('gbook_01'),'javascript:history.back();','no');
	...
	public function save()
	{
			...
			if($data['fid']==0 || empty($data['neir'])) admin_msg(L('gbook_01'),'javascript:history.back();','no');

            $row=$this->db->query("SELECT id FROM ".CS_SqlPrefix."gbook where cid=1 and fid=".$data['fid']."")->row();
			if($row){
                 $this->CsdjDB->get_update('gbook',$row->id,$data);
			}else{
                 $this->CsdjDB->get_insert('gbook',$data);
			}
			echo "<script type='text/javascript'>parent.$('.webox').css({display:'none'});parent.$('.background').css({display:'none'});parent.parent.web_box(2);parent.document.location.reload();</script>";
	}
	...
	public function del()
	{
			...
			if(empty($id)) admin_msg(L('gbook_02'),'javascript:history.back();','no');  //数据不完成
            $this->CsdjDB->get_del('gbook',$id,'fid');
			$this->CsdjDB->get_del('gbook',$id);
            admin_msg(L('gbook_03'),site_url('gbook'),'ok');  //操作成功
	}
}
