...
class CI_Loader {
	...
	public function model($model, $name = '', $db_conn = FALSE)
	{
		...
		foreach ($this->_ci_model_paths as $mod_path)
		{
			...
			if ( ! file_exists(CSCMS.'app/models/'.$path.$model.'.php') && ! file_exists($mod_path.'models/'.$path.$model.'.php'))
			{
				continue;
			}
	...
	public function helper($helpers = array())
	{
		foreach ($this->_ci_prep_filename($helpers, '_helper') as $helper)
		{
			...
			{
                $helper_path=$path.'helpers/'.$helper.'.php';
				if (!file_exists($path.'helpers/'.$helper.'.php'))
				{
                      $helper_path=CSCMS.'app/helpers/'.$helper.'.php';
				}
	...
	protected function _ci_load_class($class, $params = NULL, $object_name = NULL)
	{
		...
		foreach (array(ucfirst($class), strtolower($class)) as $class)
		{
			...
			foreach ($this->_ci_library_paths as $path)
			{
				...
				{
					$filepath = CSCMS.'app/libraries/'.$subdir.$class.'.php';
				}
		        ...
		        if (is_file($dir_file)) {
			        ...
			        if (is_array($plub)) {
						  ...
						  //前台视图
							  ...
							  if(defined('HOMEPATH')){
								   if($ulog==1) $path = str_replace("\\", "/", CSCMS);
								   if(!empty($skins)){
			                           $dirs='tpl/home/'.$skins;
								   }else{
