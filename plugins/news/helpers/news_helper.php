...
function getChild($CID,$type='news_list',$zd='fid'){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
...
//获取数据总数
function getcount($table='news',$sid=0){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
...
function SearchLink($Search_Key,$fid='tags'){
	 $Search_link='http://'.Web_Url.Web_Path.'index.php/news/'; //板块地址
	 ...
	 //开启二级域名
	 $Ym_Mode=config('Ym_Mode','news'); //二级域名状态
	 $Ym_Url=config('Ym_Url','news');   //二级域名地址
	 $Web_Mode=config('Web_Mode','news');   //版块运行模式
	 if($Ym_Mode==1){
		 if($Web_Mode==2){
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/news/",$Ym_Url.Web_Path,$Search_List);
		 }else{
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/news/",$Ym_Url.Web_Path."index.php/",$Search_List);
		 }
	 }
	 if($Web_Mode==2){
