...
class Lists extends Cscms_Controller {
	...
	{
            $sql_string = "SELECT * FROM ".CS_SqlPrefix."news_list where fid=0 order by xid asc";
	...
	public function init()
	{
            ...
            $this->CsdjDB->get_update('news_list',$id,$edit);
            die($str);
	}
	...
	public function plsave()
	{
			...
			foreach ($ids as $id) {
                    ...
                    $this->CsdjDB->get_update('news_list',$id,$data);
			}
	...
	public function zhuan()
	{
			...
			$ids=implode(',', $ids);
            $this->db->query("update ".CS_SqlPrefix."news set cid=".$cid." where cid in (".$ids.")");
            admin_msg('恭喜您，操作成功~!',site_url('news/admin/lists'),'ok');  //操作成功
	}
	...
	public function edit()
	{
			...
			if($id==0){
                ...
                $row=$this->db->query("SELECT * FROM ".CS_SqlPrefix."news_list where id=".$id."")->row(); 
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $this->CsdjDB->get_insert('news_list',$data);
			}else{
                 $this->CsdjDB->get_update('news_list',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('news/admin/lists'),'ok');  //操作成功
	...
	public function del()
	{
			...
			$this->CsdjDB->get_del('news_list',$ids,'fid');
			$this->CsdjDB->get_del('news_list',$ids);
