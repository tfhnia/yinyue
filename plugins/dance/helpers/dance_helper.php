...
function getChild($CID,$type='dance_list',$zd='fid'){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
...
//获取数据总数
function getcount($table='dance',$sid=0){
	 $ci = &get_instance();
	 if (!isset($ci->db)){
        $ci->load->database();
	 }
...
function SearchLink($Search_Key,$fid='tags'){
	 $Search_link='http://'.Web_Url.Web_Path.'index.php/dance/'; //板块地址
	 ...
	 //开启二级域名
	 $Ym_Mode=config('Ym_Mode','dance'); //二级域名状态
	 $Ym_Url=config('Ym_Url','dance');   //二级域名地址
	 $Web_Mode=config('Web_Mode','dance');   //版块运行模式
	 if($Ym_Mode==1){
		 if($Web_Mode==2){
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/dance/",$Ym_Url.Web_Path,$Search_List);
		 }else{
	          $Search_List  = str_replace(Web_Url.Web_Path."index.php/dance/",$Ym_Url.Web_Path."index.php/",$Search_List);
		 }
	 }
	 if($Web_Mode==2){
