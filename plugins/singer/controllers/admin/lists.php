...
class Lists extends Cscms_Controller {
	...
	public function init()
	{
            ...
            if($ac=='zt'){ //显示
                  $edit['yid']=$sid;
				  $str=($sid==0)?'<a title="点击隐藏" href="javascript:get_cmd(\''.site_url('singer/admin/lists/init').'?sid=1\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/ok.gif" /></a>':'<a title="点击显示" href="javascript:get_cmd(\''.site_url('singer/admin/lists/init').'?sid=0\',\'zt\','.$id.');"><img align="absmiddle" src="'.Web_Path.'packs/admin/images/icon/no.gif" /></a>';
			}
            $this->CsdjDB->get_update('singer_list',$id,$edit);
            die($str);
	}
	...
	public function plsave()
	{
			...
			foreach ($ids as $id) {
                    ...
                    $this->CsdjDB->get_update('singer_list',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('singer/admin/lists'),'ok');  //操作成功
	}
	...
	public function save()
	{
			...
			if($id==0){ //新增
                 $this->CsdjDB->get_insert('singer_list',$data);
			}else{
                 $this->CsdjDB->get_update('singer_list',$id,$data);
			}
            admin_msg('恭喜您，操作成功~!',site_url('singer/admin/lists'),'ok');  //操作成功
	...
	public function del()
	{
			...
			$this->CsdjDB->get_del('singer_list',$ids,'fid');
			$this->CsdjDB->get_del('singer_list',$ids);
