...
class Mv extends Cscms_Controller {
    ...
    //内容
	public function index($fid='hits', $id = 0, $page=1)
	{
            $id = intval($id);   //ID
            $page = intval($page);   //ID
			if(preg_match("/^\d*$/",$fid)){
	        ...
	        if(config('Web_Mode')==3 && $html['show']['check']==1 && !defined('MOBILE')){
                //获取静态路径
				$Htmllink=LinkUrl('mv','id',$id,0,'singer');
			...
			unset($row['tags']);
			//装载模板并输出
			$Mark_Text=$this->CsdjTpl->plub_list($row, $id, $fid, $page, $arr, TRUE, 'mv.html', 'mv', 'singer', $row['name'],$row['name']);
			//评论
