...
class Album extends Cscms_Controller {
	...
	public function index($fid='id', $id = 0, $page=1)
	{
	        ...
	        if(config('Web_Mode')==3 && $html['album']['check']==1 && !defined('MOBILE')){
                //获取静态路径
				$Htmllink=LinkUrl('album','id',$id,0,'singer');
			...
			unset($row['tags']);
			//装载模板并输出
			$Mark_Text=$this->CsdjTpl->plub_list($row, $id, $fid, $page, $arr, TRUE, 'album.html', 'album', 'singer', $row['name'],$row['name']);
			//评论
